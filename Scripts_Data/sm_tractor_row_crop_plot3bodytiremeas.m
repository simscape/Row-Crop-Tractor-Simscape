function sm_tractor_row_crop_plot3bodytiremeas(logsoutRes)
...
simlog_t    = simlog_Veh.Values.Chassis.Body.CG.aRoll.Time;
simlog_rollVeh  = simlog_Veh.Values.Chassis.Body.CG.aRoll.Data(:);
simlog_pitchVeh = simlog_Veh.Values.Chassis.Body.CG.aPitch.Data(:);

simlog_f_t      = squeeze(simlog_Veh.Values.TireFL.Fz.Time);
simlog_fnFL     = squeeze(simlog_Veh.Values.TireFL.Fz.Data);
simlog_fnFR     = squeeze(simlog_Veh.Values.TireFR.Fz.Data);
simlog_fnRL     = squeeze(simlog_Veh.Values.TireRL.Fz.Data);
simlog_fnRR     = squeeze(simlog_Veh.Values.TireRR.Fz.Data);
