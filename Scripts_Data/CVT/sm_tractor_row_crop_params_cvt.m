function HMPST = sm_tractor_row_crop_params_cvt(VehicleData)
% Function to load parameters for the CVT

%% 1. Define parameters necessary for CVT design
wRadR = VehicleData.TireDataR.param.DIMENSION.UNLOADED_RADIUS;
HMPST.Tire.Rad = wRadR;%393.7/1000; % m

diffRatioFactor = (393.7/1000)*wRadR;
...
HMPST.Differential.Ratio = ...
    ...
    HMPST.Differential.N_Pinion/diffRatioFactor;
...
xFinal = [1.460159998718090 24 18 24 24];
...
HMPST.Veh.Mass      = VehicleData.Body.Mass; % kg
HMPST.Veh.Track     = VehicleData.SuspF.Track; % m
HMPST.Veh.Wheelbase = abs(VehicleData.Body.RA); % m
HMPST.Veh.CG.FA2CGx    = abs(VehicleData.Body.CG(1)); % m
HMPST.Veh.CG.RA2CGx    = HMPST.Veh.Wheelbase-HMPST.Veh.CG.FA2CGx; % m
HMPST.Veh.CG.FA2CGz    = abs(VehicleData.Body.CG(3)); % m
...
HMPST.Control.Kp                  = 10/100;        % -
HMPST.Control.Ki                  = 0.5/100;        % -
...
% Tires, 3D
HMPST.Tire.Contact = simscape.multibody.tirread('KT_MF_Tool_460_60_R28.tir');
